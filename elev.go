...
/*
Translated to GO
I am not totaly sure if the matrixes is equal to C but the are on the correct form, maybe they need to be transposed(?)
...
lamp_channel_matrix := [N_FLOORS][N_BUTTONS]int{[N_BUTTONS]int{LIGHT_UP1, LIGHT_DOWN1, LIGHT_COMMAND1},
...
static const int lamp_channel_matrix[N_FLOORS][N_BUTTONS] = {
  ...
  */
button_channel_matrix := [N_FLOORS][N_BUTTONS]int{[N_BUTTONS]int{FLOOR_UP1, FLOOR_DOWN1, FLOOR_COMMAND1},
...
func elev_get_floor_sensor_signal(){
	...
	//by convention in go no else is used, the function will not continue if one of the previous returns is called(?)
	return -1
}
/*
...
func elev_set_floor_indicator(floor int) {
    ...
    // Binary encoding. Ome ligth must always be on
    if floor && 0x02 {
